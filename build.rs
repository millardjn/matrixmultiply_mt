...
fn main() {
	...
	if let Ok(flags) = env::var("MATMULFLAGS") {
		...
		let other_flags = &["ftz_daz", "prefetch", "no_multithreading"];
		...
		for user_flag in user_flags.iter() {
			if arch_flags.iter().any(|flag| flag == user_flag) {
				...
				panic!(
					"matrixmultiply_mt: Environment variable MATMULFLAGS contained unrecognised flag: {}",
					user_flag
				);
			}
		}
