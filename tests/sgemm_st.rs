...
use matrixmultiply_mt::{dgemm_st, sgemm_st};
...
impl Gemm for f32 {
	unsafe fn gemm(
		...
		sgemm_st(m, k, n, alpha, a, rsa, csa, b, rsb, csb, beta, c, rsc, csc)
	}
}
...
impl Gemm for f64 {
	unsafe fn gemm(
		...
		dgemm_st(m, k, n, alpha, a, rsa, csa, b, rsb, csb, beta, c, rsc, csc)
	}
}
	...
	F: Gemm + Float,
{
	...
	for (i, (x, y)) in a.iter().zip(&c).enumerate() {
		if x != y {
			...
			panic!("mismatch at index={}, x: {}, y: {} (matrix input M={}, N={})", i, x, y, m, n);
		}
	}
